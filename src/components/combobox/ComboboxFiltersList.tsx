...
export default function ComboboxFiltersList({isFetchingNextPage,hasNextPage,fetchNextPage,isFetching,items,dataCount,setValue,values}: IInfiniteVirtual) {
        ...
        <>
            <ScrollArea.Root>
                <ScrollArea.Viewport style={{
                    ...
                    <CommandList className={'mr-4'} style={{
                        ...
                        {rowVirtualizer?.getVirtualItems().map((virtualItem) => {
                            ...
                            return (
                                ...
                                >
                                    <FaCheck
                                        className={cn(" mr-[5px] h-4 w-4", values?.some(val => val === listItem.id) ? "opacity-100" : "opacity-0")}/>
                                    {listItem?.name}
                    ...
                    <div className="flex justify-center items-center">
                        {items.length === 0 &&
                            <span className={'text-text flex justify-center   py-2 items-center text-base'}>Ничего не найдено</span>}
                    </div>
                </ScrollArea.Viewport>
